...
int main(int argc, char** argv) {
    ...
    CLI::App app{"WireKrak - Kraken Trade Subscription Example\n"
        ...
        "Example runs until interrupted with Ctrl+C"};
    ...
    app.add_option("-s,--symbol", symbol, "Trading symbol (e.g. BTC/USD)")->check(symbol_validator)->default_val(symbol);
    app.add_option("--snapshot", snapshot, "Request trade snapshot")->check(CLI::IsMember({true, false}))->default_val(snapshot);
    //app.add_flag("--no-snapshot", snapshot, "Disable trade snapshot")->default_val(snapshot)->disable_flag_override();
    ...
    app.footer("Let's enjoy trading with WireKrak!");
    ...
    client.on_status([&](const status::Update& update) {
        WK_INFO("[STATUS] received update: " << update.str() << "");
    });

    // Register regection handler
    client.on_rejection([&](const rejection::Notice& notice) {
        WK_WARN("[REJECTION] received notice: " << notice.str() << "");
    });
    ...
    client.subscribe(trade::Subscribe{.symbols = {symbol}, .snapshot = snapshot},
    ...
    // Ctrl+C received
    client.unsubscribe(trade::Unsubscribe{.symbols = {symbol}});
    if (double_sub) {
        client.unsubscribe(trade::Unsubscribe{.symbols = {symbol}});
    }
