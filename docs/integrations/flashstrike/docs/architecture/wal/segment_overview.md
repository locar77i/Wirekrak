# Flashstrike WAL Storage Architecture — Overview
...
📄 **Detailed spec:**  
👉 [`WAL segment header`](./segment/header.md)
...
📄 Detailed spec:  
👉 [`WAL segment block_header`](./segment/block_header.md)
...
👉 [`WAL segment block`](./segment/block.md)
...
- rebuilds state from segment 0 → N  
...
Flashstrike’s Write-Ahead Log subsystem provides:
