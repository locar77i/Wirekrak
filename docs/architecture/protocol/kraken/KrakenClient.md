# Wirekrak Kraken Streaming Client

## Overview

The **Wirekrak Kraken Streaming Client** implements the Kraken WebSocket API v2 on top of Wirekrak’s
...
The client is designed for low-latency systems and deterministic event processing, making it suitable
...
The Kraken client intentionally exposes **protocol-level concepts** only (subscriptions, acks,
...
The client is composed of several layers:
...
```cpp
wirekrak::protocol::kraken::Client<wirekrak::transport::winhttp::WebSocket> client;

client.on_status([](const status::Update& s) {
...
This client demonstrates how to build a **robust, reusable protocol SDK** on top of a generic streaming core, while maintaining strong typing,
deterministic behavior, and clean separation between infrastructure and business logic.

---

⬅️ [Back to README](../../../../README.md#kraken-client)
