...
static bool parse(std::string_view json, book::Update& out) {
    simdjson::dom::parser parser;
    auto doc = parser.parse(json);
    assert(!doc.error());
    return parser::book::update::parse(doc.value(), out);
}
...
void test_book_update_success_bids_only() {
    ...
    assert(upd.bids.size() == 1);
    assert(upd.asks.empty());
...
void test_book_update_success_asks_only() {
    ...
    book::Update upd{};
    assert(parse(json, upd));
    assert(upd.asks.size() == 1);
    assert(upd.bids.empty());
